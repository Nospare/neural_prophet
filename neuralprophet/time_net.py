...
class TimeNet(pl.LightningModule):
    ...
    def __init__(
        ...
        if self.events_dims is not None:
            ...
            expand_mask[len(self.quantiles) // 2, :] = torch.tensor(self.additive_constraint_mask, dtype=torch.float32)
            ...
            expand_mask[len(self.quantiles) // 2, :] = torch.tensor(self.multiplicative_constraint_mask, dtype=torch.float32)
